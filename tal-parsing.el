...
(defun tal/check-node-frontier (node input A i j k l)
  "Determine whether NODE ∈ A[i, j, k, l] for the given INPUT.
...
it)."
  (let ((frontier (bt/leaves node)))
    ;; check the first part (i -> j)
    (and
     (loop for x from i to (1- j)
           for el in (mapcar #'bt/node-symbol frontier)
           if (not (eq el (elt input x)))
           do (return nil)
           collect x)
     ;; check the second part (k -> l)
     (loop for x from k to (1- l)
           for el in (mapcar #'bt/node-symbol frontier)
           if (not (eq el (elt input x)))
           do (return nil)
           collect x))))
  ...
  (loop for m from k to (1- l)
        ...
        (loop for p from m to l
              ...
              (loop for node in left
                    ...
                    do (when (bt/node-p (bt/node-parent node))
                         (pushnew (bt/node-parent node)
                                  (elt (elt (elt (elt A i)
                                                 j)
                                            k)
                                       l)))))))
  ...
  (loop for m from j to (1- l)
        ...
        (loop for p from j to l
              ...
              (loop for node in left
                    ...
                    (when (bt/node-p (bt/node-parent node))
                      (pushnew (bt/node-parent node)
                               (elt (elt (elt (elt A i)
                                              j)
                                         j)
...
in the INPUT."
  (let ((terminal-leaves (append
    ...
    (loop for i from 0 to (1- (length input))
          ...
          (mapc (lambda (node)
                  (when (and (bt/node-p node)
                             (eq (elt input i)
                                 (bt/node-symbol node)))
                    (pushnew node (elt (elt (elt (elt A i)
                                                 (1+ i))
                                            (1+ i))
                                       (1+ i)))))
                terminal-leaves)))
