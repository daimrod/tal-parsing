...
can be generated by TAG."
  (let* ((n (length input))
    ...
    (let ((ret (loop for j from 0 to n
                     for intersection = (intersection (tal/tag-initial-trees tag)
                                                      (elt (elt (elt (elt A 0)
                                                           ...
                                                           n))
                     if intersection
                     do (return 'recognized)
                     finally (return 'not-recognized))))
      (if noninteractive
          (prin1 ret)
        ret))))
...
;; L = { w c^2n / n >= 0 }
;; w is made of a's and b's and there is n a's and n b's
(defvar tal/tag-example-2 (make-tal/tag
                           :initial-trees (bt/make-trees '((S a (T (T b c) c))))
                           :auxiliary-trees (bt/make-trees '((S a (T (T b (T S c)) c))))))
